...
def get_FACE_data():
    #read the data
    train_data = np.load('Data/sorted_train_data.npy')
    # Split 1000 examples for validation
    X_train = train_data[1000:32000,:784]
    y_train = np.argmax(train_data[1000:32000,784:],axis=1)
    X_val = train_data[:1000,:784]
    y_val= np.argmax(train_data[:1000,784:],axis=1)
...
def Emotion_model(X,y,is_training):
    ...
    W1=tf.get_variable(shape=[7*7*128,1024],name = 'W1')
...
print('Training')
run_model(sess,y_out,mean_loss,X_train,y_train,50,100,100,train_step,False)
print('Validation')
run_model(sess,y_out,mean_loss,X_val,y_val,1,64)
model_saver.save(sess,"/home/linux/Documents/DLExercises/Emotion_Recognition/saved_models/Emotion_model2")
sess.close()         